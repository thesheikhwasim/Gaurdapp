import { IonButtons, IonList, IonContent, IonHeader, IonMenuButton, IonPage, IonTitle, IonToolbar, IonImg, IonButton, IonInput, IonCard, IonCardContent, IonCardHeader, IonCardSubtitle, IonCardTitle, IonCol, IonGrid, IonRow, IonModal, IonLabel, IonItem, IonSelect, IonSelectOption, useIonToast, IonLoading } from '@ionic/react';
...
const Dashboard: React.FC = () => {
  ...
  const checkActivity = () => {
    captureLocation().then(() => {
      if (latitude !== '' && longitude !== '') {
        ...
        axios.post('https://guard.ghamasaana.com/admin/matchlat.php', formData)
          .then(response => {
            console.log('Activity check response:', response.data);
          })
          .catch(error => {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(activityInterval); // Cleanup interval on component unmount
        ...
        <IonGrid className='ion-margin ion-text-center'>
          <IonRow>
            <IonCol size="4" size-md="4" size-lg="4">
              <IonButton color="secondary" onClick={() => setShowModal(true)}>{t('ticket')}</IonButton>
            </IonCol>
