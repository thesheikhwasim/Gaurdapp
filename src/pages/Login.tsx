...
import { Checkvalidtoken, DutyMovementApi, GlobalLogout, ValidateSimcardnumber } from '../utility/Globalapis';
...
const Login: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    //  hitCheckPermissionsForGeolocation();
      ...
      formData.append('simone', 'NA');
      formData.append('simtwo',  'NA');
      ...
      if (simCardsFromDevice && simCardsFromDevice.length > 0) {
        ...
        setsim1no(simArray[0]);
        setsim2no(simArray[1]);
        formData.append('simone', simArray[0]);
        formData.append('simtwo',  simArray[1]);
      //  alert(simArray[0]+" Sim1 "+simArray[1]+" Sim2 "+mobileNumber+" mobile ");
   
      }   
  //  alert(sim1no+" Sim1 "+sim2no+" Sim2 "+mobileNumber+" mobile ");
        ...
        if (response) {
          ...
          if (response.success) {
            ...
            setBtnEnabled(true);
            localStorage.setItem('guard_movementhit', response.employee_data.guard_movementhit);
            localStorage.setItem('mobileno', response.employee_data.reg_mobile_no);
          ...
          setBtnEnabled(true);
          present({
            message: `Some permission missing on this device Please check permission and try again!`,
            duration: 2000,
            position: 'bottom',
          });
  ...
  const handleForgotPassword = () => {
    ...
    if (simCardsFromDevice && simCardsFromDevice.length > 0) {
      ...
      if ((simArray[0] != '' && (simArray[0] === ('91' + reqforgotmobile) || simArray[0] === (reqforgotmobile)  || simArray[0] === ('+91' + reqforgotmobile))) || (simArray[1] != "" && (simArray[1] === ('91' + reqforgotmobile) || simArray[1] === (reqforgotmobile) || simArray[1] === ('+91' + reqforgotmobile)))) {
    ...
    axios
      ...
      present({
        message: `Your Mobile Number not Matching with Device Mobile Number. Please use the same device`,
        duration: 2000,
        position: 'bottom',
      });
      ...
      present({
        message: `Not able to read you Device Mobile Number. Please connect with Admin.`,
        duration: 2000,
        position: 'bottom',
      });
